...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (window.Echo) {
    ...
    try {
      var _loop = function _loop() {
        ...
        Echo.channel('server-status.{{ $server->id }}'(_templateObject || (_templateObject = _taggedTemplateLiteral(["server-status.", ""])), server.getAttribute('server-id'))).listen('ServerStatusUpdated', function (event) {
  ...
  function toggleOnlineVisibility(root) {
    ...
    var _iterator5 = _createForOfIteratorHelper(root.getElementsByClassName('data-hide-online')),
        _step5;
