        ...
        <div class="row server-list">
            @foreach ($servers as $server)
                <div class="col-12 mb-4">
                    <div class="server bg-light bg-{{ $server->last_query_result->online() ? \SquadMS\Foundation\Helpers\LevelHelper::levelToClass($server->last_query_result->level()) : 'no-map' }} bg-cover bg-center" server-id="{{ $server->id }}" server-level-bg="bg-{{ $server->last_query_result->online() ? \SquadMS\Foundation\Helpers\LevelHelper::levelToClass($server->last_query_result->level()) : 'no-map' }}">
